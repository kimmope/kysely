...
import Rowmappers.PastQandARowMapper;
...
// CHECK IF USER EXISTS OR CREATE NEW IF IT DOESNT
	...
	public List<PastQandA> getPastQandAs(int uid) {			// Hae lista aikaisemmista kysymys-vastauspareista
		return jdbcTemplate.query("SELECT u.uid, u.qid, q.question, u.timeOfAnswer, u.answer FROM user_answers u INNER JOIN questions q ON u.qid=q.qid WHERE u.uid=? ORDER BY u.timeOfAnswer DESC",PastQandARowMapper.pastQandARowMapper,uid);	// List queryllä 
	}
	public PastQandA getPastQandA(int uid, int qid) {		// Hae yksi kysymys-vastauspari
		return jdbcTemplate.queryForObject("SELECT u.uid, u.qid, q.question, u.timeOfAnswer, u.answer FROM user_answers u INNER JOIN questions q ON u.qid=q.qid WHERE u.uid=? AND q.qid=? ORDER BY u.timeOfAnswer DESC",PastQandARowMapper.pastQandARowMapper,uid,qid);	//	Objekti queryForObjectillä
	}
...
// CALCULATING FIGURES FOR ANSWER STATISTICS-CLASS
	...
	public List<Statistics> getCountryStatistics(int qid) {
		return jdbcTemplate.query("(SELECT COUNT(*) AS amount, ROUND(AVG(ua.answer),0) AS mean,\n" + 
				"ROUND(AVG(ua.answer),0) AS medi,\n" + 
				"(SELECT answer FROM user_answers WHERE (answer='AO01' OR answer='AO02' OR answer='AO03' OR answer='AO04' OR answer='AO05') AND user_answers.qid = ua.qid GROUP BY answer ORDER BY COUNT(answer) DESC LIMIT 1) AS classmode,\n" + 
				"(SELECT answer FROM user_answers WHERE answer REGEXP '^-?[0-9]+$' AND user_answers.qid = ua.qid GROUP BY answer ORDER BY COUNT(answer) DESC LIMIT 1) AS mode,\n" + 
	...
	public List<Statistics> getProvinceStatistics(int qid) {
		return jdbcTemplate.query("(SELECT COUNT(*) AS amount, ROUND(AVG(ua.answer),0) AS mean,\n" + 
				"ROUND(AVG(ua.answer),0) AS medi,\n" + 
				"(SELECT answer FROM user_answers WHERE (answer='AO01' OR answer='AO02' OR answer='AO03' OR answer='AO04' OR answer='AO05') AND user_answers.qid = ua.qid GROUP BY answer ORDER BY COUNT(answer) DESC LIMIT 1) AS classmode,\n" + 
				"(SELECT answer FROM user_answers WHERE answer REGEXP '^-?[0-9]+$' AND user_answers.qid = ua.qid GROUP BY answer ORDER BY COUNT(answer) DESC LIMIT 1) AS mode,\n" + 
